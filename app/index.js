...
async function runDashaApp(config, phone, forward) {
  if (forward === undefined)
      forward = null;
  let sdk = new dasha.DashaSdk({
      url: "app.beta.dasha.ai",
      apiKey: "6Qp3WzXmrt0LSIxBrA8gOiau7MYl2DxZDbKnbbm9ppg"
    });
  ...
  if (phone !== null) {
      ...
      let job = await app.startJob(phone, "audio", { data: { phone: phone, forward }, debugEvents: createLogger(), rpcHandler: { reportEvent: reportEvent } });
      ...
      app.onJob({
          startingJob: async(serverId, id, incomingData) => {
              ...
              const job = { data: { phone: "", forward: forward }, debugEvents: createLogger(), rpcHandler: { reportEvent: reportEvent } };
